...
| B5 | Outcome tag = AgentRunStatus mapped via `Outcomes.Map(...)` | Snake_case lowercase per existing convention. The outcome vocabulary `"success"` / `"failure"` / `"cancelled"` is the **canonical fleet-wide cross-service contract**: used by Phase 3.H per-dispatch histograms in this repo, Phase 3.J per-run histograms in this repo, AND qwen Workflow's run-duration metering (qwen PR #19 aligns to the same set). Operators write a single cross-service PromQL query on the `outcome` tag and get consistent values for the same conceptual outcome across every service in the fleet. Run-specific additions `"cap_reached"` / `"loop_detected"` stay run-specific — no per-dispatch equivalent in 3.H and no cross-service equivalent in Workflow. In-flight states (Planning/Running) defensively fall through to `"unknown"`. |
...
- **Don't drift from the canonical outcome vocabulary.** The values `"success"` / `"failure"` / `"cancelled"` (+ Assistant-run-specific `"cap_reached"` / `"loop_detected"`) are the canonical fleet-wide outcome tags. Phase 3.H per-dispatch histograms, Phase 3.J per-run histograms, AND qwen Workflow's run-duration metering (qwen PR #19) all align to this set. Cross-service dashboards depend on this consistency; any new metric emitting an `outcome` tag MUST use these strings, sourced from the `AgentTelemetry.Outcomes` constants — never inline a string literal.
