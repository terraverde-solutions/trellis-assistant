...
4. ~~**Tool turns rendered as `ChatRole.System` for next-turn LLM context**~~ — **resolved post-3.A.2 retrofit++.** Trellis.Core PR #15 widened `ChatRole` with `Tool=3` + `"tool"` wire string; the Assistant sibling PR flipped `ConversationOrchestrator.ToCoreRole` from `AssistantTurnRole.Tool → ChatRole.System` (workaround) to `→ ChatRole.Tool` (canonical). The model's tool-trained head now sees the canonical role label when reconstructing history for the next-turn LLM call. Pinned by `ToCoreRole_ToolTurn_MapsToChatRoleTool` (mutation pin: a regression to System would pass the BudgetVerdict-level contract but degrade tool-call quality silently). The optional `name` field on Ollama's tool-message wire shape (`{role: "tool", content, name?}`) is still NOT carried — `ChatMessage` doesn't have a `ToolName?` field; that's a separate widening for QA-telemetry-driven follow-up if quality improves further.
